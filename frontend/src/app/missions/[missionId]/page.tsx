    ...
    return (
      <div className="max-w-4xl mx-auto py-10">
        <div className="rounded-xl border border-rose-800 bg-rose-950/50 p-6 text-xs font-mono text-rose-300">
          [Mission Error] {error || 'Assurance mission data not found.'}
        </div>
      </div>
    );
  }
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-slate-800 pb-4">
        <div>
          <div className="flex items-center gap-3">
            ...
            <span className="rounded bg-sky-950 px-2 py-0.5 text-xs font-mono font-bold text-sky-300 border border-sky-800">
              {missionData.status}
            </span>
        ...
        <button
          onClick={loadData}
          className="inline-flex items-center gap-1.5 rounded-lg border border-slate-800 bg-slate-900 px-3.5 py-1.5 text-xs font-semibold text-slate-300 hover:bg-slate-850 self-start sm:self-auto"
        >
          <RefreshCw className="h-3.5 w-3.5" /> Refresh Mission
        </button>
      </div>
      ...
      <div className="pt-2">
        ...
        {activeTab === 'semantic' && (
          semanticSec.status === 'SUCCEEDED' ? (
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-8 text-center text-xs text-slate-400 font-mono">
              Status: {semanticSec.status} — {semanticSec.reason || 'Semantic analysis omitted or not available.'}
            </div>
          )
        )}
        ...
        {activeTab === 'impact' && (
          impactSec.status === 'SUCCEEDED' ? (
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-8 text-center text-xs text-slate-400 font-mono">
              Status: {impactSec.status} — {impactSec.reason || 'Dependency DAG analysis omitted or not available.'}
            </div>
          )
        )}
        ...
        {activeTab === 'experiments' && (
          experimentsSec.status === 'SUCCEEDED' ? (
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-8 text-center text-xs text-slate-400 font-mono">
              Status: {experimentsSec.status} — {experimentsSec.reason || 'Boundary experiments omitted or not available.'}
            </div>
          )
        )}
        ...
        {activeTab === 'recon' && (
          reconSec.status === 'SUCCEEDED' ? (
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-8 text-center text-xs text-slate-400 font-mono">
              Status: {reconSec.status} — {reconSec.reason || 'Reconciliation & RCA omitted or not available.'}
            </div>
          )
        )}
        ...
        {activeTab === 'blast' && (
          blastSec.status === 'SUCCEEDED' ? (
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-8 text-center text-xs text-slate-400 font-mono">
              Status: {blastSec.status} — {blastSec.reason || 'Blast radius analysis omitted or not available.'}
            </div>
          )
        )}
        ...
        {activeTab === 'remediation' && (
          remediationSec.status === 'SUCCEEDED' && remediationSec.data ? (
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-8 text-center text-xs text-slate-400 font-mono">
              Status: {remediationSec.status} — {remediationSec.reason || 'Remediation & Revalidation omitted or not required.'}
            </div>
          )
        )}
        ...
        {activeTab === 'agent' && (
          agentSec.status === 'SUCCEEDED' ? (
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-8 text-center text-xs text-slate-400 font-mono">
              Status: {agentSec.status} — {agentSec.reason || 'Agent execution timeline omitted.'}
            </div>
          )
        )}
      </div>
